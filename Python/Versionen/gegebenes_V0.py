"""
Live-Messung mit NI-DAQmx auf dB-Skala, Outlier-Removal, mean/std Anzeige und Aufnahme
- Live-Plot des clean Signals in dB
...
V0 = 0.5               # Referenzspannung [V] für dB-Berechnung
...
plt.ion()
fig, ax = plt.subplots()
ax.set_title("Live clean signal in dB")
...
try:
    ...
    while plt.fignum_exists(fig.number):
        ...
        # --- dB-Skala ---
        clean_db = 20 * np.log10(np.maximum(np.abs(clean)/V0, 1e-12))
        current_mean = np.mean(clean_db)
        current_std  = np.std(clean_db)

        # --- Plot aktualisieren ---
        line.set_xdata(np.arange(len(clean_db)))
        line.set_ydata(clean_db)
        ax.set_xlim(0, len(clean_db))
        ax.set_ylim(min(clean_db)-1, max(clean_db)+1)

        # --- Text aktualisieren ---
        text_mean.set_text(f"Mean: {current_mean:.4f} dB")
        text_std.set_text(f"Std:  {current_std:.4f} dB")

        # --- Aufnahme prüfen ---
        if is_recording:
            record_buffer.extend(clean_db)
            ...
            if elapsed >= RECORD_DURATION:
                ...
                saved_measurements.append({"mean_dB": m, "std_dB": s})
                print(f"Messung aufgenommen: mean={m:.4f} dB, std={s:.4f} dB")
                is_recording = False
